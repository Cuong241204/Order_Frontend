...
- 💳 **Thanh toán**: Hỗ trợ Stripe và VNPay
...
├── backend/
│   ├── src/
│   │   ├── config/        # Cấu hình (database, stripe, vnpay, email)
...
### Payment
- `POST /api/payment/stripe/create-intent` - Tạo Stripe payment
- `POST /api/payment/vnpay/create-url` - Tạo VNPay URL
...
- `test-stripe-payment.sh` - Test cấu hình Stripe
