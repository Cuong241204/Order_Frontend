...
const TableManagement = () => {
  ...
  const loadTables = () => {
    ...
    if (stored) {
      ...
      // Default 5 tables
      const defaultTables = [
        { id: 1, number: 'Bàn 1', capacity: 4, status: 'available', qrCode: generateQRCode(1) },
        { id: 2, number: 'Bàn 2', capacity: 4, status: 'available', qrCode: generateQRCode(2) },
        { id: 3, number: 'Bàn 3', capacity: 4, status: 'available', qrCode: generateQRCode(3) },
        { id: 4, number: 'Bàn 4', capacity: 4, status: 'available', qrCode: generateQRCode(4) },
        { id: 5, number: 'Bàn 5', capacity: 4, status: 'available', qrCode: generateQRCode(5) }
      ];
    ...
    if (editingTable) {
      const updated = tables.map(table =>
        table.id === editingTable.id
          ? { ...formData, id: editingTable.id, qrCode: generateQRCode(editingTable.id) }
          : table
      );
      saveTables(updated);
    } else {
